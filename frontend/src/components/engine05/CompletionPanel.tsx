...
export function CompletionPanel() {
  ...
  const handleRegisterEngine = async (assetId: number, engine: string) => {
    try {
      if (engine === '1') {
        const res = await engine05Api.registerInEngine1(assetId);
        showToast('دارایی در موتور ۱ ثبت شد — در صف ارزیابی قرار گرفت ✅');
      ...
      const msg = err?.response?.data?.error || err?.message || 'خطا';
      showToast(msg, 'error');
    }
  };
                        ...
                        <div key={asset.id} className="border border-gray-200 rounded-lg p-3">
                          ...
                          <div className="flex items-center gap-2">
                            {asset.registered_in_engine_1 ? (
                              ...
                              <button
                                onClick={() => handleRegisterEngine(asset.id, '1')}
                                className="flex items-center gap-2 bg-[#04241D] text-white rounded-lg px-4 py-2 text-xs hover:opacity-90"
                              >
                                <FileText className="w-4 h-4" />
                                ثبت در ارزیابی دارایی‌ها
