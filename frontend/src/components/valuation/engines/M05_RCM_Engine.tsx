...
export function M05_RCM_Engine({ 
  ...
  // تبدیل عدد به فارسی (برای مرحله و نفر-ماه)
  ...
  const chartData = waterfallData.map((item: any) => ({
    name: `مرحله ${toPersianNumber(item.step)}`,
  ...
  return (
    <div className="space-y-6 font-[family-name:var(--font-vazir)]" dir="rtl">
      ...
      <section className="relative overflow-hidden rounded-[30px] border border-slate-200/80 bg-white p-5 shadow-[0_14px_45px_rgba(15,23,42,0.055)] sm:p-7">
        <div className="pointer-events-none absolute -right-24 -top-28 h-64 w-64 rounded-full bg-emerald-100/65 blur-3xl" />
        <div className="pointer-events-none absolute -left-20 bottom-0 h-48 w-48 rounded-full bg-teal-100/40 blur-3xl" />
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <div className="mb-1.5 flex flex-wrap items-center gap-2">
                <span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-black text-emerald-700">M-05</span>
                <span className="rounded-full border border-slate-200 bg-white px-2.5 py-1 text-[10px] font-bold text-slate-500">Replacement Cost Method</span>
                <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-bold text-slate-600">روش هزینه</span>
              </div>
              <h2 className="text-xl font-black text-slate-900 sm:text-2xl">نتایج ارزش‌گذاری روش RCM</h2>
              <p className="mt-1 max-w-2xl text-xs leading-6 text-slate-500 sm:text-sm">
                ارزش دارایی بر مبنای هزینه بازسازی دارایی معادل، تعدیلات سربار و سود توسعه‌دهنده و کسر استهلاک محاسبه شده است.
              </p>
            </div>
          </div>

          {hasData && (
            <span className="inline-flex self-start rounded-full border border-emerald-100 bg-emerald-50 px-3.5 py-2 text-xs font-bold text-emerald-700 lg:self-auto">
              داده از دیتابیس
            </span>
          )}
        </div>
      ...
      {!data && (
        <section className="relative overflow-hidden rounded-[28px] border border-dashed border-slate-300 bg-white px-6 py-12 text-center shadow-[0_8px_30px_rgba(15,23,42,0.035)]">
          ...
          <h3 className="mt-4 text-base font-black text-slate-800">آماده محاسبه هزینه جایگزینی</h3>
          <p className="mx-auto mt-2 max-w-md text-xs leading-6 text-slate-500">برای دریافت داده‌ها و اجرای مدل RCM، فرآیند محاسبه را آغاز کنید.</p>
          <Button
              ...
              <CardContent className="p-0">
                <div className="flex items-center gap-3 border-b border-slate-100 px-5 py-5 sm:px-6">
                  ...
                  <div>
                    ...
                    <p className="mt-0.5 text-[10px] text-slate-400">نفر-ماه، نرخ ماهانه و هزینه مستقیم تیم بازسازی</p>
                  </div>
                </div>
                    ...
                    <table className="w-full min-w-[620px] border-collapse text-sm">
                      <thead>
                        <tr className="bg-slate-50 text-[10px] font-bold text-slate-500">
                          ...
                          <th className="p-3 text-center">نفر-ماه</th>
                          <th className="p-3 text-right">نرخ ماهانه</th>
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {laborDetails.map((item: any, index: number) => (
                          <tr key={index} className="bg-white transition-colors hover:bg-slate-50/70">
                            ...
                            <td className="p-3 text-center font-black text-slate-600">{toPersianNumber(item.person_months)}</td>
                            <td className="p-3 text-right text-slate-600">{formatNumber(item.monthly_rate)}</td>
                            <td className="p-3 text-right font-black text-dark-green">{formatNumber(item.person_months * item.monthly_rate)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
          ...
          <Card className="overflow-hidden rounded-[28px] border border-slate-200/80 bg-white shadow-[0_10px_35px_rgba(15,23,42,0.045)]">
            <CardContent className="p-0">
              <div className="flex flex-col gap-4 border-b border-slate-100 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                <div>
                  <h4 className="text-sm font-black text-slate-800">آبشار هزینه جایگزینی</h4>
                  <p className="mt-1 text-[10px] text-slate-400">اثر افزایشی هزینه‌ها و کاهشی استهلاک بر ارزش تجمعی دارایی</p>
                </div>
                <div className="flex flex-wrap gap-3 text-[10px] font-bold text-slate-500">
                  <span className="flex items-center gap-1.5"><i className="h-2.5 w-2.5 rounded-full bg-green-500" />افزایش</span>
                  <span className="flex items-center gap-1.5"><i className="h-2.5 w-2.5 rounded-full bg-red-500" />کاهش</span>
                  <span className="flex items-center gap-1.5"><i className="h-2.5 w-2.5 rounded-full bg-teal-500" />ارزش نهایی</span>
                </div>
              </div>
              <div className="p-4 sm:p-6">
                <div style={{ width: '100%', height: 410 }} dir="ltr">
                  <ResponsiveContainer>
                    <BarChart data={chartData} margin={{ top: 48, right: 25, left: 10, bottom: 25 }} barGap={8}>
                      <CartesianGrid strokeDasharray="4 4" vertical={false} stroke="#e2e8f0" />
                      <XAxis dataKey="name" tick={{ fontSize: 11, fill: '#64748b', fontFamily: 'var(--font-vazir)' }} axisLine={false} tickLine={false} />
                      <YAxis tick={{ fontSize: 10, fill: '#94a3b8', fontFamily: 'var(--font-vazir)' }} tickFormatter={(value) => formatNumber(value)} axisLine={false} tickLine={false} />
                      <Tooltip
                      ...
                      <Bar dataKey="value" radius={[6, 6, 0, 0]} barSize={48}>
                        {chartData.map((entry: any, index: number) => (
                          <Cell key={`cell-${index}`} fill={getColor(entry)} opacity={entry.isFinal ? 1 : 0.86} />
                        ))}
          ...
          <Card className="overflow-hidden rounded-[28px] border border-slate-200/80 bg-white shadow-[0_10px_35px_rgba(15,23,42,0.045)]">
            <CardContent className="p-0">
              <div className="border-b border-slate-100 px-5 py-5 sm:px-6">
                ...
                <p className="mt-1 text-[10px] text-slate-400">ردیابی مرحله‌به‌مرحله تعدیلات و ارزش تجمعی مدل RCM</p>
              </div>
              <div className="overflow-x-auto p-4 sm:p-5">
                <div className="overflow-hidden rounded-2xl border border-slate-200">
                  <table className="w-full min-w-[760px] border-collapse text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {waterfallData.map((item: any, index: number) => {
                        ...
                        return (
                          <tr key={index} className={`transition-colors ${isFinal ? 'bg-teal-50/60' : 'bg-white hover:bg-slate-50/70'}`}>
                            <td className="p-3 text-center font-black text-slate-600">{toPersianNumber(item.step)}</td>
                            <td className="p-3 font-bold text-slate-700">{item.title}</td>
                            <td className={`p-3 font-black ${isFinal ? 'text-teal-700' : isDecrease ? 'text-red-600' : isIncrease ? 'text-green-600' : 'text-slate-600'}`}>{displayAmount}</td>
                            <td className={`p-3 ${isFinal ? 'text-base font-black text-teal-700' : 'font-bold text-slate-600'}`}>{formatNumber(item.cumulative)}</td>
                          </tr>
                        );
                      })}
                    </tbody>
          ...
          <section className="relative overflow-hidden rounded-[30px] bg-gradient-to-br from-[#073f35] via-dark-green to-[#0b6b58] p-6 text-white shadow-[0_18px_50px_rgba(5,75,63,0.18)] sm:p-7">
            ...
            <div className="relative flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
              <div>
                ...
                <p className="mt-2 text-3xl font-black tracking-tight sm:text-4xl">{formatNumber(displayFinal)} <span className="text-base font-bold text-emerald-100/70">ریال</span></p>
                <p className="mt-2 text-[10px] text-emerald-100/60">ارزش پس از اعمال تمام تعدیلات هزینه و استهلاک</p>
              </div>
