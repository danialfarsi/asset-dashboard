...
import {
  Target, FileText, Image as ImageIcon, Video, Music, Trash2,
...
export default function StrategicPlanningPage() {
  ...
  const loadUploads = async () => {
    setLoading(true);
    try {
      const res = await viamUploadsApi.list({ section: 'strategic_planning' });
  ...
  const stats = {
    ...
    archive: uploads.filter((u) => u.file_type === 'archive').length,
  };
          ...
          <div className="relative flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
            <div className="flex items-start gap-4">
              <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-white/10 ring-1 ring-white/20 backdrop-blur-sm">
                <Target className="h-7 w-7" style={{ color: BRAND.goldSoft }} />
              </div>
              <div className="pt-0.5">
                ...
                <p className="mt-1.5 text-sm text-white/65">
                  آپلود، سازمان‌دهی و مدیریت اسناد راهبردی
                </p>
              </div>
            </div>
          ...
          <Card className="rounded-3xl border-2 border-dashed border-slate-200 bg-white/60">
            <CardContent className="p-16 text-center">
              ...
              <p className="mt-1 text-xs text-slate-400">
                {uploads.length === 0
                  ? 'اولین سند راهبردی خود را از بخش بالا بارگذاری کنید'
