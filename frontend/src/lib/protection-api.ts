...
export const protectionApi = {
  ...
  // 🆕 دریافت اطلاعات کامل با asset_id (نه profile_id)
  getFullByAsset: async (assetId: number): Promise<FullProtectionData> => {
    // ۱. اول profile رو با asset_id پیدا کن
    const listRes = await api.get<{ results: ProtectionProfile[]; count: number }>(
      `/intangible/protection/?asset=${assetId}`
    );
    
    const profiles = listRes.data.results || [];
    if (profiles.length === 0) {
      // پروفایل وجود نداره — خطای خاص بده که بتونیم تشخیص بدیم
    ...
    // ۲. حالا full رو با profile_id بگیر
    const fullRes = await api.get<FullProtectionData>(
      `/intangible/protection/${profileId}/full/`
    );
  ...
  createForAsset: (assetId: number) => 
    api.post<{profile_id: number; created: boolean; archetype: string; asset_name: string; asset_uid: string}>(
      '/intangible/protection/create_for_asset/',
      { asset_id: assetId }
    ),
};
