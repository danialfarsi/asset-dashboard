// ============================================
// 🔥 مپ کامل کدهای اختصاری برای فرانت‌اند
// ============================================

export const CODE_TO_ASSET_TYPE: Record<string, string> = {
  // ========== فرم‌های اصلی ==========
  'BRD': 'BRAND',
  'CON': 'CONTRACT',
  'BMC': 'BMC',
...
export const getAssetTypeCodeFromUid = (uid: string): string | null => {
  if (!uid) return null;
  const parts = uid.split('-');
  if (parts.length >= 3) {
    const code = parts[2];
    return CODE_TO_ASSET_TYPE[code] || null;
  }
  return null;
};
