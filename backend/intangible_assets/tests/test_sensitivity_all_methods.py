"""
تست STEP 6 (تحلیل حساسیت) برای ۹ روش ارزش‌گذاری
"""
...
from intangible_assets.services.sensitivity_engine import SensitivityEngine
...
class TestSensitivityAllMethods(TestCase):
    """تست STEP 6 برای همه ۹ روش"""
    ...
    def run_sensitivity(self, case, method_id, base_value, inputs):
        """اجرای تحلیل حساسیت"""
        case_data = {
            'method_id': method_id,
    ...
    def test_sensitivity_m01(self):
        ...
        result, _ = self.run_sensitivity(case, 'M-01', step4.final_value, inputs)
        
        drivers = result.get('key_drivers', [])
        print(f"  ✅ M-01: {len(drivers)} متغیر کلیدی")
        for d in drivers[:3]:
            print(f"     - {d.get('driver_name')}: {d.get('impact_percent', 0):.2f}%")
        self.assertGreater(len(drivers), 0, "M-01 باید حداقل یک متغیر کلیدی داشته باشد")

    def test_sensitivity_m02(self):
        print("\n🧪 تست STEP 6 - M-02: MEEM")
        ...
        result, _ = self.run_sensitivity(case, 'M-02', step4.final_value, inputs)
        
        drivers = result.get('key_drivers', [])
        print(f"  ✅ M-02: {len(drivers)} متغیر کلیدی")
        for d in drivers[:3]:
            print(f"     - {d.get('driver_name')}: {d.get('impact_percent', 0):.2f}%")
        self.assertGreater(len(drivers), 0, "M-02 باید حداقل یک متغیر کلیدی داشته باشد")

    def test_sensitivity_m03(self):
        print("\n🧪 تست STEP 6 - M-03: DCF")
        ...
        result, _ = self.run_sensitivity(case, 'M-03', step4.final_value, inputs)
        
        drivers = result.get('key_drivers', [])
        print(f"  ✅ M-03: {len(drivers)} متغیر کلیدی")
        for d in drivers[:3]:
            print(f"     - {d.get('driver_name')}: {d.get('impact_percent', 0):.2f}%")
        self.assertGreater(len(drivers), 0, "M-03 باید حداقل یک متغیر کلیدی داشته باشد")

    def test_sensitivity_m04(self):
        print("\n🧪 تست STEP 6 - M-04: WWM")
        ...
        result, _ = self.run_sensitivity(case, 'M-04', step4.final_value, inputs)
        
        drivers = result.get('key_drivers', [])
        print(f"  ✅ M-04: {len(drivers)} متغیر کلیدی")
        for d in drivers[:3]:
            print(f"     - {d.get('driver_name')}: {d.get('impact_percent', 0):.2f}%")
        self.assertGreater(len(drivers), 0, "M-04 باید حداقل یک متغیر کلیدی داشته باشد")

    def test_sensitivity_m05(self):
        print("\n🧪 تست STEP 6 - M-05: RCM")
        ...
        result, _ = self.run_sensitivity(case, 'M-05', step4.final_value, inputs)
        
        drivers = result.get('key_drivers', [])
        print(f"  ✅ M-05: {len(drivers)} متغیر کلیدی")
        for d in drivers[:3]:
            print(f"     - {d.get('driver_name')}: {d.get('impact_percent', 0):.2f}%")
        self.assertGreater(len(drivers), 0, "M-05 باید حداقل یک متغیر کلیدی داشته باشد")

    def test_sensitivity_m06(self):
        print("\n🧪 تست STEP 6 - M-06: RPCM")
        ...
        result, _ = self.run_sensitivity(case, 'M-06', step4.final_value, inputs)
        
        drivers = result.get('key_drivers', [])
        print(f"  ✅ M-06: {len(drivers)} متغیر کلیدی")
        for d in drivers[:3]:
            print(f"     - {d.get('driver_name')}: {d.get('impact_percent', 0):.2f}%")
        self.assertGreater(len(drivers), 0, "M-06 باید حداقل یک متغیر کلیدی داشته باشد")

    def test_sensitivity_m07(self):
        print("\n🧪 تست STEP 6 - M-07: TWC")
        ...
        result, _ = self.run_sensitivity(case, 'M-07', step4.final_value, inputs)
        
        drivers = result.get('key_drivers', [])
        print(f"  ✅ M-07: {len(drivers)} متغیر کلیدی")
        for d in drivers[:3]:
            print(f"     - {d.get('driver_name')}: {d.get('impact_percent', 0):.2f}%")
        self.assertGreater(len(drivers), 0, "M-07 باید حداقل یک متغیر کلیدی داشته باشد")

    def test_sensitivity_m08(self):
        print("\n🧪 تست STEP 6 - M-08: CTM")
        ...
        result, _ = self.run_sensitivity(case, 'M-08', step4.final_value, inputs)
        
        drivers = result.get('key_drivers', [])
        print(f"  ✅ M-08: {len(drivers)} متغیر کلیدی")
        for d in drivers[:3]:
            print(f"     - {d.get('driver_name')}: {d.get('impact_percent', 0):.2f}%")
        self.assertGreater(len(drivers), 0, "M-08 باید حداقل یک متغیر کلیدی داشته باشد")

    def test_sensitivity_m09(self):
        print("\n🧪 تست STEP 6 - M-09: MMM")
        ...
        result, _ = self.run_sensitivity(case, 'M-09', step4.final_value, inputs)
        
        drivers = result.get('key_drivers', [])
        print(f"  ✅ M-09: {len(drivers)} متغیر کلیدی")
        for d in drivers[:3]:
            print(f"     - {d.get('driver_name')}: {d.get('impact_percent', 0):.2f}%")
        self.assertGreater(len(drivers), 0, "M-09 باید حداقل یک متغیر کلیدی داشته باشد")

    def test_all_sensitivity(self):
        """اجرای تست STEP 6 برای همه ۹ روش"""
        print("\n" + "="*60)
        print("🚀 تست STEP 6 (تحلیل حساسیت) برای همه ۹ روش")
        print("="*60)
        
        self.test_sensitivity_m01()
        self.test_sensitivity_m02()
        self.test_sensitivity_m03()
        self.test_sensitivity_m04()
        self.test_sensitivity_m05()
        self.test_sensitivity_m06()
        self.test_sensitivity_m07()
        self.test_sensitivity_m08()
        self.test_sensitivity_m09()
        
        print("\n" + "="*60)
        print("✅ STEP 6 برای همه ۹ روش با موفقیت تست شد!")
        print("="*60)
