...
class DiscoveryAnalyzer:
    ...
    def analyze(self):
        ...
        return {
            'best_template': self._serialize_template(best_template, errors),
            'errors': errors,
            'alternative': self._serialize_template(alternative, []) if alternative else None,
            'summary': self.generate_summary(best_template, errors, alternative)
        }
    
    def _serialize_template(self, template, errors=None):
        if not template:
            return None
        return {
            'id': template.id,
            'name': template.item_name,
            'organization_type': template.organization_type.display_name if template.organization_type else None,
            'category': template.category,
            'valuation_method': template.valuation_method,
            'total_score': 15,
            'max_score': 29,
            'match_percentage': self.calculate_match_percentage(template),
            'asset_type_id': template.asset_type.id if template.asset_type else None
        }
    
    def calculate_match_percentage(self, template):
        ...
        return int((total_match / total_items * 100)) if total_items > 0 else 0
    ...
    def generate_summary(self, best_template, errors, alternative):
        ...
        if critical_errors:
            # تولید پیام‌های خوانا با توضیحات کامل از QUESTION_DESC
            suggestions = []
            for e in critical_errors:
                key = e['key'].lower()
                # دریافت توضیحات از QUESTION_DESC
                desc = self.QUESTION_DESC.get(key, e.get('description', key))
                expected = 'بله' if e.get('expected', 1) == 1 else 'خیر'
                user = 'بله' if e.get('user_value', 0) == 1 else 'خیر'
                # اضافه کردن توضیحات کامل به جای کلید
                suggestions.append(f"• {desc} (وضعیت فعلی: {user} | باید: {expected})")
            
            # پیام اصلی
            if len(critical_errors) == 1:
                message = f'دارایی شما به "{best_template.item_name}" نزدیک است، فقط یک مورد نیاز به اصلاح دارد:'
            elif len(critical_errors) <= 3:
                message = f'دارایی شما به "{best_template.item_name}" نزدیک است، این موارد را اصلاح کنید:'
            else:
                message = f'برای تطابق بهتر با "{best_template.item_name}"، این موارد ضروری را بررسی کنید:'
            ...
            if normal_errors:
                suggestions = []
                for e in normal_errors[:5]:
                    key = e['key'].lower()
                    desc = self.QUESTION_DESC.get(key, e.get('description', key))
                    suggestions.append(f"• {desc}")
                
                if len(normal_errors) > 5:
                    suggestions.append(f"• ... و {len(normal_errors) - 5} مورد دیگر")
                
                return {
                    'type': 'good',
                    'message': f'✅ دارایی شما با "{best_template.item_name}" تطابق خوبی دارد، اما چند نکته جزئی وجود دارد:',
                    'suggestions': suggestions,
                    'alternative_message': None
                }
            else:
