...
class ValuationStep3ViewSet(viewsets.ModelViewSet):
    ...
    # ============================================
    # 🔥🔥🔥 ENDPOINT دریافت شواهد - مهم
    # ============================================
    @action(detail=True, methods=['get'])
    def evidences(self, request, pk=None):
        """دریافت لیست شواهد یک STEP 3"""
        try:
            step3 = self.get_object()
            evidences = step3.evidences.all()
            serializer = ValuationStep3EvidenceSerializer(evidences, many=True)
    ...
    # 🔥🔥🔥 ENDPOINT حذف شاهد
    # ============================================
    @action(detail=True, methods=['delete'], url_path='evidence/(?P<evidence_id>[0-9]+)')
    def delete_evidence(self, request, pk=None, evidence_id=None):
        """حذف یک شاهد خاص"""
        try:
            step3 = self.get_object()
            evidence = step3.evidences.get(id=evidence_id)
            ...
            evidence.delete()
            return Response(
                {'message': 'شاهد با موفقیت حذف شد'},
                status=status.HTTP_204_NO_CONTENT
            )
        except ValuationStep3Evidence.DoesNotExist:
            return Response(
                {'error': 'شاهد یافت نشد'},
